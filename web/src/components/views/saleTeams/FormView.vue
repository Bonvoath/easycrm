    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb font-kulen">
                <li class="breadcrumb-item" aria-current="page"><router-link to="/saleteam">{{$t('sale_team')}}</router-link></li>
...
<script>
    export default {
        ...
        methods: {
            findById(id){
                this.$api().post('saleteam/find', { id: id }).then(res => {
                    if(this.$isValid(res)){
            ...
            dbSave(){
                ...
                this.$api().post('saleteam/save', this.model).then(res => {
                    if(this.$isValid(res)){
                        this.$router.push('/saleteam');
                    }
                });
            },
            dbUpdate(){
                ...
                this.$api().post('saleteam/update', req).then(res => {
                    if(this.$isValid(res)){
                        this.$router.push('/saleteam');
                    }
                });
